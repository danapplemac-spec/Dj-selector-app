...
class DJApiController:
    ...
    def analyzeTrack(self, path: str) -> None:
        """Analyze a track if analysis dependencies are installed.

        Emits onAnalysisError if optional dependencies are missing.
        """
        ...
        except Exception as exc:  # optional dependency failure
            self.events.emit("onAnalysisError", path=path, error=str(exc))
            return
        ...
        self.events.emit(
            ...
            path=path,
            bpm=details["bpm"],
            musicalKey=details["musical_key"],
        )
    ...
    def togglePlay(self, deck: str) -> None:
        ...
        self.events.emit(
            "onTransportState",
            deck=deck,
            state="playing" if deck_state.is_playing else "paused",
        )
